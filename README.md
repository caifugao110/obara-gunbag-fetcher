...
- **桌面优先加载策略**：清单文件、日志文件、输出目录均支持桌面优先路径，配置变更后路径自动重新计算
...
- **自动加载**：启动时自动加载 `config.ini` 和默认清单文件，清单文件优先从桌面加载，桌面不存在时回退到程序目录
...
> **💡 桌面优先加载图号清单**：程序启动时会自动检测桌面上是否存在 `original_list_file` 配置项指定的清单文件（如 `Original file list.txt`）。若桌面存在该文件，程序会优先加载桌面版本；仅当桌面文件不存在时，才回退使用程序同目录下的清单文件。此机制方便用户在桌面维护一份常用清单而无需复制到程序目录。
...
程序在以下场景中均采用「桌面优先」策略加载文件路径：

| 场景 | 行为说明 |
|---|---|
| **启动自动加载清单** | 优先检测桌面是否存在 `original_list_file` 指定的文件，存在则直接加载；不存在时回退到程序同目录 |
| **配置变更后重新加载** | 切换模式、修改配置后，重新按桌面优先逻辑计算清单路径，保留已加载的桌面路径不丢失 |
| **图号管理保存** | 开启 `list_on_desktop` 时，保存路径切换至桌面；关闭时保存回原路径 |
| **输出目录计算** | 开启 `prefer_desktop` 时，输出目录指向 `桌面/gunbag`；关闭时指向 `程序目录/gunbag` |
| **日志文件路径** | 开启 `log_on_desktop` 时，日志输出至桌面；关闭时输出至程序目录 |

所有路径计算均在 `load_configuration()` 和 `apply_runtime_paths()` 中统一处理，确保普通模式和仕样号模式下行为一致。
...
2. **启动程序** — 双击 `obara-gunbag-fetcher.exe`，程序自动加载：
   - `config.ini` 配置文件
   - `Original file list.txt` 清单文件（优先从桌面加载，桌面不存在时回退到程序目录）
...
```
obara-gunbag-fetcher/
├── app.py                  # GUI 主程序，包含全部核心逻辑（单文件约 2600 行）
├── assets/
│   └── app.ico             # 应用图标
├── scripts/
...
| 文件/目录 | 说明 |
|---|---|
| `app.py` | 主程序入口，包含 GUI 界面、配置管理、文件扫描、打包处理等全部逻辑（单文件约 2600 行） |
