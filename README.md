...
- 同时包含找到的 2D 和 3D 文件
- 支持 3D 文件按清单重命名（可选）
- 部分文件缺失时仍会打包已有文件（仅 2D 或仅 3D 缺失时），若 2D 和 3D 均未找到则跳过该清单项
- 自动清空并重建输出目录
...
- 顶部分别显示标题、主题切换、GitHub 链接、使用说明、更新日志、关于
...
- **在线更新日志**：从 Gitee Releases API 实时获取最近 5 条版本记录
- **在线帮助文档**：内置使用说明窗口，从 Gitee 加载 README.md 内容
- **关于对话框**：显示项目名称、作者、协议、项目链接
...
| 配置项 | 类型 | 说明 | 代码回退默认值 |
|---|---|---|---|
| `output_dir_name` | string | 本地输出目录名称 | `output` |
...
3. **配置检查** — 确认源目录路径正确，可通过"配置管理"按钮进行增删改。

4. **开始打包** — 点击"开始批量打包"按钮，等待完成。

5. **查看结果** — 在输出目录（默认 `gunbag`）中可看到：
   - 按清单命名的 ZIP 文件
   - CSV 日志文件记录每个清单项的找到/缺失情况
...
> **注意**：若开启"3D 按清单重命名"，ZIP 内的 3D 文件名会使用清单项名称而非原始文件名。
...
| SRTX-2C14701L | SRTX-2C14701L.zip | 无 | D:\2D源\SRTX-2C14701L.PDF | 3D | success |
...
- **更新日志**：点击右上角"更新日志"按钮，从 Gitee Releases API 获取最近 5 条版本记录
- **使用说明**：点击"使用说明"按钮，在线加载 README.md 内容
...
| `config.ini` | 用户配置文件，包含路径、性能参数、源目录等 |
...
- `process_item()`：处理单个清单项，查找文件并打包为 ZIP
- `ensure_output_directory()`：清空并重建输出目录
- 使用 `ThreadPoolExecutor` 实现并发处理

### GUI 组件

- `GunbagFetcherApp`：主应用窗口
